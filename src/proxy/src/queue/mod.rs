...
macro_rules! let_settings {
    ($settings: ident) => {
        ...
        // map_err(|err|
        //     anyhow!("failed to read settings: {}", err)
        // )?;
        // let $settings = $settings.as_ref().unwrap();
        let $settings = $settings.as_ref().unwrap();
        // .ok_or_else(||
        //     anyhow!("failed to unwrap settings")
        // )?;
    };
}
...
macro_rules! start_proxy_check {
    ($fut_queue: expr, $source_fetch_count: ident, $proxy_url_queue_to_check: expr, $proxy_url_check_count: expr, $source_queue_to_fetch: expr) => {
        if $source_queue_to_fetch.len() == 0 && $source_fetch_count == 0 && $proxy_url_queue_to_check.len() == 0 && $proxy_url_check_count == 0 {
            ...
            // let_settings!(settings);
            // let (sources, same_time_request_max_count, proxy_timeout_secs) = {
            //     let settings = settings::SINGLETON.read().unwrap().as_ref().unwrap();
            //     (settings.sources, settings.same_time_request_max_count, settings.proxy_timeout_secs)
            // };
            // let sources = settings::SINGLETON.read().unwrap().as_ref().unwrap().sources;
//             let same_time_request_max_count = settings::SINGLETON.read().unwrap().as_ref().unwrap().same_time_request_max_count;
//             let proxy_timeout_secs = settings::SINGLETON.read().unwrap().as_ref().unwrap().proxy_timeout_secs;
// // settings::SINGLETON.read().unwrap().unwrap().sources.some();
// // settings::SINGLETON.read().unwrap().as_ref().some();
//             let settings = settings::SINGLETON.read().unwrap();//.as_ref();
            let_settings!(settings);
        ...
        let own_ip = 
            ...
            // None => {
                ...
                match $own_ip_opt.take() {
                    ...
                    Some(own_ip) => {
                        // let settings = settings::SINGLETON.read().unwrap().as_ref().unwrap();
                        if Instant::now().duration_since(own_ip.last_update).as_secs() < settings.own_ip_fresh_duration_secs as u64 {
                            own_ip
                        } else {
                            info!("get_own_ip . . .");
                            match get_own_ip().await {
...
mod check_proxy {
    ...
    pub async fn run(arg: Arg) -> Ret {
        ...
        let_settings!(settings);
        let status = match super::get_ip(arg.client, settings.echo_service_url.as_ref()).await {
...
pub async fn get_own_ip() -> Result<OwnIp> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()?
    ;
    let ip = get_ip( client, settings.echo_service_url.as_ref()).await?;
    Ok(OwnIp{
        ip, 
        last_update: Instant::now(),
    })
...
mod reuse_proxy {
    ...
    pub async fn run(arg: Arg) -> Ret {
        let_settings!(settings);
        // let settings = settings::SINGLETON.read().unwrap().as_ref().unwrap();
        tokio::time::delay_for(Duration::from_millis(settings.proxy_rest_duration_millis)).await;
        arg
    }
}
